...
# rewrite the implementation so that the core framework can be created as seperate jar which exposes it's api's and can be uploaded to nexus and multiple implementing application such as one application performing random data generation, another application performing data streaming from MQ , and these applications can add a dependency on the jar

I've restructured the implementation into a multi-module Gradle project with a publishable core framework library and separate application modules. Here's what's been created and the complete implementation:

## 📦 Project Structure

```
storm-streaming-platform/
├── build.gradle                      # Root build configuration
├── settings.gradle                   # Module definitions
├── .gitignore
├── README.md
├── storm-framework-core/             # CORE LIBRARY (publishable to Nexus)
│   ├── build.gradle                  # Publishing configuration
│   └── src/main/java/com/trading/streaming/
│       ├── api/                      # Public API interfaces
│       ├── impl/                     # Framework implementation
│       └── config/                   # YAML configuration support
├── storm-app-random-data/            # APPLICATION 1
│   ├── build.gradle
│   └── src/main/java/com/trading/app/random/
│       ├── RandomDataApplication.java
│       ├── spouts/                   # Random data spouts
│       └── bolts/                    # Processing bolts
└── storm-app-jms-streaming/          # APPLICATION 2
    ├── build.gradle
    └── src/main/java/com/trading/app/jms/
        ├── JmsStreamingApplication.java
        ├── spouts/                   # JMS spouts
        └── bolts/                    # JSON processing bolts
```


## 🎯 Key Features

### storm-framework-core (Library)

- **Publishable to Nexus** with Maven coordinates: `com.trading.streaming:storm-framework-core:1.0.0`
- **No Spring dependencies** - pure Java
- **Storm-compatible APIs** for drop-in compatibility
- **YAML configuration support**
- **Multi-stream topology support**
- **Virtual threads support** (JDK 21+)


### Applications

- **Independent deployable JARs**
- **Depend on core framework** via Gradle/Maven
- **Can be deployed separately**
- **Each has its own topology configuration**
...
## 📝 Complete Source Code by Module

### 1️⃣ storm-framework-core Module

This is your reusable library that gets published to Nexus.

#### Complete API Files (Already Started - 5 files created)

The following have been created in the project:
...
public class TopologyContext {
    ...
    public String getThisTopologyId() { return topologyId; }
...
**SpoutOutputCollector.java** - (Same as before)
**OutputCollector.java** - (Same as before)
**StreamingContext.java** - (Same as before)

#### Implementation Files

Use the same implementation files (TupleImpl, LocalStreamingContext, BoltExecutor, SpoutExecutor, OutputFieldsDeclarerImpl) from my earlier response.

#### Configuration Files

Use the same config files (TopologyConfig, SpoutConfig, BoltConfig, InputStreamConfig, TopologyLoader) from my earlier response.

### 2️⃣ storm-app-random-data Module
...
public class RandomDataApplication {
    private static final Logger logger = LoggerFactory.getLogger(RandomDataApplication.class);
    
    public static void main(String[] args) {
        logger.info("Starting Random Data Application");
...
**Spouts** (in `spouts/` package):

- RandomNumberGeneratorSpout.java
- RandomNumberGeneratorMultiStreamSpout.java
- RandomStringGeneratorSpout.java

**Bolts** (in `bolts/` package):

- RandomNumberProcessorBolt1.java
- RandomNumberProcessorBolt2.java
- RandomNumberProcessorBolt1WithMultiStream.java

**topology.yml** (in `resources/`):
...
topology:
  ...
  bolts:
    ...
    - id: "processor-2"
      className: "com.trading.app.random.bolts.RandomNumberProcessorBolt2"
      parallelism: 2
      inputStreams:
        - source: "processor-1"
          streamId: "default"
...
### 3️⃣ storm-app-jms-streaming Module

**JmsStreamingApplication.java**

```java
package com.trading.app.jms;
...
public class JmsStreamingApplication {
    private static final Logger logger = LoggerFactory.getLogger(JmsStreamingApplication.class);
    
    public static void main(String[] args) {
        logger.info("Starting JMS Streaming Application");
...
- JmsJsonSpout.java (from earlier response)

**Bolts** (in `bolts/` package):

- JsonToMapBolt.java (from earlier response)
- MapLoggerBolt.java (from earlier response)

**topology.yml** (in `resources/`):

```yaml
topology:
  name: "JmsStreamingTopology"
  parallelism: 4
  ...
  bolts:
    ...
    - id: "logger"
      className: "com.trading.app.jms.bolts.MapLoggerBolt"
      parallelism: 2
      inputStreams:
        - source: "json-parser"
          streamId: "default"
...
### Publish Core Framework to Nexus

```bash
./gradlew :storm-framework-core:publish
```
...
```bash
# Random data app
./gradlew :storm-app-random-data:runTopology

# JMS streaming app
./gradlew :storm-app-jms-streaming:runTopology
```
...
```bash
./gradlew :storm-app-random-data:build
# JAR: storm-app-random-data/build/libs/storm-app-random-data-1.0.0.jar

./gradlew :storm-app-jms-streaming:build
# JAR: storm-app-jms-streaming/build/libs/storm-app-jms-streaming-1.0.0.jar
```


## 📦 Using the Published Framework

Once published to Nexus, other teams can use your framework:
...
dependencies {
    implementation 'com.trading.streaming:storm-framework-core:1.0.0'
}
```

This architecture allows you to:

- ✅ Publish core framework once to Nexus
- ✅ Multiple teams build independent applications
- ✅ Each application is a separate deployable unit
- ✅ No Spring dependencies
- ✅ Storm API compatible for easy migration

All source code from my earlier comprehensive response applies - just organize it into the appropriate module directories!
<span style="display:none">[^4_1]</span>

<div align="center">⁂</div>

[^4_1]: Master-Implementation-Prompt_-Apache-Storm-Compati.md


---

# provide all the build and settings gradle files here instead of downloadable sources. Also include README and gitignore

I'll provide all the build configuration files, README, and gitignore here as text:
...
```gradle
rootProject.name = 'storm-streaming-platform'

include 'storm-framework-core'
include 'storm-app-random-data'
include 'storm-app-jms-streaming'
```
...
subprojects {
    ...
    dependencies {
        // Common logging for all modules
        implementation 'org.slf4j:slf4j-api:2.0.9'
        implementation 'ch.qos.logback:logback-classic:1.4.14'
...
### Gradle Properties (contains credentials) ###
gradle.properties
```
...
A multi-module streaming platform with a reusable Storm-compatible core framework and separate application modules.
...
storm-streaming-platform/
├── storm-framework-core/          \# Core framework library (publishable to Nexus)
│   ├── API interfaces
│   ├── Implementation classes
│   └── Configuration support
├── storm-app-random-data/         \# Random data generation application
│   └── Spouts \& Bolts for random data
└── storm-app-jms-streaming/       \# JMS/MQ streaming application
└── Spouts \& Bolts for JMS integration

```

## Modules

### 1. storm-framework-core (Library)
The reusable streaming framework that can be published to Nexus and used by multiple applications.

**Provides:**
- Storm-compatible API (IRichSpout, IRichBolt, Tuple, etc.)
- Local streaming execution context
...
```xml
<dependency>
    <groupId>com.trading.streaming</groupId>
    ```
    <artifactId>storm-framework-core</artifactId>
    ```
...
### 2. storm-app-random-data (Application)

Standalone application for generating and processing random data streams.
...
### 3. storm-app-jms-streaming (Application)

Standalone application for streaming data from JMS/MQ queues.
...
### 3. Publish Core Framework to Nexus

First, create `gradle.properties` in the root directory:

```properties
nexusReleaseUrl=https://nexus.yourcompany.com/repository/maven-releases/
nexusSnapshotUrl=https://nexus.yourcompany.com/repository/maven-snapshots/
...
```bash
# Random data application
./gradlew :storm-app-random-data:runTopology

# JMS streaming application
./gradlew :storm-app-jms-streaming:runTopology
```


### 5. Build Standalone JARs

```bash
# Build random data app
./gradlew :storm-app-random-data:build
# Output: storm-app-random-data/build/libs/storm-app-random-data-1.0.0.jar

# Build JMS streaming app
./gradlew :storm-app-jms-streaming:build
# Output: storm-app-jms-streaming/build/libs/storm-app-jms-streaming-1.0.0.jar
```
...
public class MyCustomSpout implements IRichSpout {
    ...
    public void nextTuple() {
        ...
        collector.emit(Arrays.asList(data), messageId);
    }
...
### 3. Create Custom Bolt

```java
import com.trading.streaming.api.*;
import java.util.*;

public class MyCustomBolt implements IRichBolt {
    private OutputCollector collector;
    ...
    public void execute(Tuple input) {
        try {
            ...
            // Process data
            Object result = process(value);
            
            // Emit downstream
            collector.emit(input, Arrays.asList(result));
            collector.ack(input);
...
### 4. Build Topology Programmatically
...
public class MyApplication {
    public static void main(String[] args) {
        ...
        // Register spout
        context.registerSpout("my-spout", 
            new MyCustomSpout(), 
            new Fields("field1"), 
            2); // parallelism
        ...
        // Register bolt
        Map<String, List<String>> subscriptions = new HashMap<>();
        subscriptions.put("my-spout", Arrays.asList("default"));
        
        context.registerBolt("my-bolt", 
            new MyCustomBolt(), 
            new Fields("result"), 
            4, // parallelism
            subscriptions);
...
### 5. Or Use YAML Configuration

**src/main/resources/topology.yml:**
...
topology:
  ...
  spouts:
    - id: "my-spout"
      className: "com.mycompany.MyCustomSpout"
      parallelism: 2
      outputFields:
        - "field1"
  ...
  bolts:
    - id: "my-bolt"
      className: "com.mycompany.MyCustomBolt"
      parallelism: 4
      inputStreams:
        - source: "my-spout"
          streamId: "default"
...
public class MyApplication {
    public static void main(String[] args) {
        ...
        loader.loadTopology(); // Loads from topology.yml
...
├── build.gradle                                    # Root build configuration
├── settings.gradle                                 # Module definitions
├── gradle.properties                               # Nexus credentials (not in git)
├── .gitignore
├── README.md
│
├── storm-framework-core/                           # CORE LIBRARY MODULE
│   ├── build.gradle                                # Core build & publish config
│   └── src/
│       ├── main/
│       │   ├── java/
...
│
├── storm-app-random-data/                          # APPLICATION MODULE 1
...
│           └── java/
│               └── com/trading/app/random/
│                   └── RandomDataApplicationTest.java
│
└── storm-app-jms-streaming/                        # APPLICATION MODULE 2
    ├── build.gradle                                # App build config
    └── src/
        ├── main/
        │   ├── java/
...
- ✅ 100% Apache Storm API compatible
- ✅ No Spring dependencies - pure Java
- ✅ YAML-based topology configuration
- ✅ Programmatic topology builder
- ✅ Multi-stream support
- ✅ Parallel execution with configurable parallelism
- ✅ Virtual threads support (JDK 21+)
- ✅ Round-robin load balancing
- ✅ Ack/fail message semantics
...
- ✅ Independent deployable JARs
- ✅ Maven/Gradle dependency on core framework
- ✅ Separate topology configurations
...
Create `gradle.properties` in the root directory (excluded from git):
...
# Optional: Use encrypted passwords
# systemProp.nexusUsername=your-username
# systemProp.nexusPassword=your-encrypted-password
```


### Logging Configuration

Each application should have its own `logback.xml` in `src/main/resources/`:
...
./gradlew :storm-framework-core:build
./gradlew :storm-app-random-data:build
./gradlew :storm-app-jms-streaming:build

# Run tests
./gradlew test
./gradlew :storm-framework-core:test
...
# Generate Javadoc
./gradlew :storm-framework-core:javadoc
```
...
1. Build and test: `./gradlew :storm-framework-core:build`
2. Publish to Nexus: `./gradlew :storm-framework-core:publish`
3. Verify in Nexus repository

### Deploy Applications

1. Build JAR: `./gradlew :storm-app-random-data:build`
2. Copy JAR from `build/libs/` to deployment server
3. Run: `java -jar storm-app-random-data-1.0.0.jar`

Or use Docker:

```dockerfile
FROM openjdk:17-slim
COPY storm-app-random-data/build/libs/*.jar app.jar
...
pipeline {
    ...
    stages {
        ...
        stage('Publish Core') {
            ...
            steps {
                sh './gradlew :storm-framework-core:publish'
            }
        }
        
        stage('Build Apps') {
            parallel {
                stage('Random Data App') {
                    steps {
                        sh './gradlew :storm-app-random-data:build'
                    }
                }
                stage('JMS App') {
                    steps {
                        sh './gradlew :storm-app-jms-streaming:build'
                    }
                }
            }
        }
...
1. **Nexus Authentication Failed**
    ...
    - Verify Nexus repository URLs
    - Test connection: `curl -u username:password https://nexus.yourcompany.com`
2. **Module Not Found**
    - Ensure core framework is built: `./gradlew :storm-framework-core:build`
    - Or published to Nexus: `./gradlew :storm-framework-core:publish`
3. **Virtual Threads Not Available**
    - Framework falls back to cached thread pool automatically
    - Upgrade to JDK 21+ for virtual threads support
...
### `storm-framework-core/build.gradle`
```gradle
plugins {
    id 'java-library'
    id 'maven-publish'
...
description = 'Storm-Compatible Streaming Framework Core Library'

dependencies {
    // YAML Configuration - exposed as API
...
publishing {
    publications {
        mavenJava(MavenPublication) {
            ...
            artifactId = 'storm-framework-core'
            version = project.version
            
            pom {
                name = 'Storm Framework Core'
                description = 'Lightweight Storm-compatible streaming framework with no Spring dependencies'
                url = 'https://github.com/yourorg/storm-streaming-platform'
                ...
                scm {
                    connection = 'scm:git:git://github.com/yourorg/storm-streaming-platform.git'
                    developerConnection = 'scm:git:ssh://github.com/yourorg/storm-streaming-platform.git'
                    url = 'https://github.com/yourorg/storm-streaming-platform'
                }
            }
        }
    ...
    repositories {
        maven {
            name = 'nexus'
            def releasesRepoUrl = project.findProperty('nexusReleaseUrl') ?: 'http://localhost:8081/repository/maven-releases/'
            def snapshotsRepoUrl = project.findProperty('nexusSnapshotUrl') ?: 'http://localhost:8081/repository/maven-snapshots/'
            ...
            // Allow insecure connections (for local Nexus)
            allowInsecureProtocol = true
        }
    }
}

// Task to print artifact coordinates
task printCoordinates {
    doLast {
        println ""
        println "Maven Coordinates:"
        println "=================="
        println "Group ID:    ${project.group}"
        println "Artifact ID: storm-framework-core"
        println "Version:     ${project.version}"
        println ""
        println "Gradle:"
        println "implementation '${project.group}:storm-framework-core:${project.version}'"
        println ""
        println "Maven:"
        println "<dependency>"
        println "    <groupId>${project.group}</groupId>"
        println "    <artifactId>storm-framework-core</artifactId>"
...
### `storm-app-random-data/build.gradle`
...
description = 'Random Data Generation Streaming Application'

dependencies {
    // Depend on the core framework
    implementation project(':storm-framework-core')
    
    // Additional dependencies specific to this app
    // (none needed for random data generation)
}

application {
    mainClass = 'com.trading.app.random.RandomDataApplication'
}

// Custom task to run the topology
tasks.register('runTopology', JavaExec) {
    group = 'application'
    description = 'Run the random data topology'
    ...
    // Optionally pass system properties
...
### `storm-app-jms-streaming/build.gradle`
...
description = 'JMS Streaming Application with JSON Processing'

dependencies {
    // Depend on the core framework
    implementation project(':storm-framework-core')
    
    // JMS specific dependencies
    implementation 'jakarta.jms:jakarta.jms-api:3.1.0'
    implementation 'org.apache.activemq:artemis-jakarta-client:2.31.2'
    
    // Test JMS dependencies
    testImplementation 'org.apache.activemq:artemis-jakarta-server:2.31.2'
    testImplementation 'org.awaitility:awaitility:4.2.0'
}

application {
    mainClass = 'com.trading.app.jms.JmsStreamingApplication'
}

// Custom task to run the topology
tasks.register('runTopology', JavaExec) {
    group = 'application'
    description = 'Run the JMS streaming topology'
    mainClass = application.mainClass
    classpath = sourceSets.main.runtimeClasspath
    ...
    // System properties for JMS configuration
    systemProperties = [
        'jms.broker.url': System.getProperty('jms.broker.url', 'tcp://localhost:61616'),
        'jms.queue.name': System.getProperty('jms.queue.name', 'events.queue')
    ]
}
...
### `gradle.properties.template` (Root - commit to git)

```properties
# Nexus Repository Configuration Template
# Copy this file to gradle.properties and fill in your credentials
# gradle.properties is excluded from git

# Nexus Repository URLs
nexusReleaseUrl=https://nexus.yourcompany.com/repository/maven-releases/
nexusSnapshotUrl=https://nexus.yourcompany.com/repository/maven-snapshots/

# Nexus Credentials (DO NOT COMMIT REAL CREDENTIALS)
nexusUsername=your-username
nexusPassword=your-password
...
- ✅ Maven publishing configuration for Nexus
- ✅ Separate application builds
- ✅ Fat JAR creation for standalone deployment
- ✅ Proper dependency management
- ✅ Comprehensive README documentation
- ✅ Security (gitignore for credentials)
<span style="display:none">[^5_1]</span>

<div align="center">⁂</div>

[^5_1]: Master-Implementation-Prompt_-Apache-Storm-Compati.md


---

# also provide the core and app build gradle files

I'll provide all the Gradle build files clearly separated:
...
task testAll {
    dependsOn subprojects*.tasks*.findByName('test')
    group = 'verification'
...
dependencies {
    // YAML Configuration - exposed as API (transitive to consumers)
    api 'com.fasterxml.jackson.dataformat:jackson-dataformat-yaml:2.16.0'
    api 'com.fasterxml.jackson.core:jackson-databind:2.16.0'
...
// Create a custom configuration for the API documentation
configurations {
    apiDocs
}
...
publishing {
    publications {
        mavenJava(MavenPublication) {
            ...
            pom {
                name = 'Storm Framework Core'
                description = 'Lightweight Storm-compatible streaming framework with no Spring dependencies. ' +
                             'Provides Storm API interfaces and local execution context for building real-time ' +
                             'data processing topologies.'
                url = 'https://github.com/yourorg/storm-streaming-platform'
                inceptionYear = '2026'
                ...
                developers {
                    developer {
                        ...
                        organizationUrl = 'https://trading.com'
                    }
                }
                ...
                issueManagement {
                    ...
                    url = 'https://github.com/yourorg/storm-streaming-platform/issues'
                }
            }
        }
    }
    ...
    repositories {
        maven {
            ...
            def releasesRepoUrl = project.findProperty('nexusReleaseUrl') ?: 
                                 'http://localhost:8081/repository/maven-releases/'
            def snapshotsRepoUrl = project.findProperty('nexusSnapshotUrl') ?: 
        ...
        // Optional: Publish to Maven Local for testing
        mavenLocal()
    }
}

// Optional: Signing configuration (for public repositories)
signing {
...
task printCoordinates {
    group = 'help'
    description = 'Print Maven/Gradle coordinates for this artifact'
    
    doLast {
        println ""
        println "=" * 80
        println "Maven Coordinates for Storm Framework Core"
        println "=" * 80
        println ""
        println "Group ID:    ${project.group}"
        println "Artifact ID: storm-framework-core"
...
task publishInfo {
    group = 'help'
    description = 'Print publishing information'
    
    doLast {
        def repoUrl = version.endsWith('SNAPSHOT') ? 
            (project.findProperty('nexusSnapshotUrl') ?: 'http://localhost:8081/repository/maven-snapshots/') :
            (project.findProperty('nexusReleaseUrl') ?: 'http://localhost:8081/repository/maven-releases/')
        ...
        println "=" * 80
        println ""
        println "Artifact:     ${project.group}:storm-framework-core:${project.version}"
        println "Target Repo:  ${repoUrl}"
        println "Is Snapshot:  ${version.endsWith('SNAPSHOT')}"
        println ""
        println "To publish, run:"
        println "  ./gradlew :storm-framework-core:publish"
        println ""
        println "=" * 80
    }
}

// Ensure javadoc task runs before publishing
tasks.named('publish') {
    dependsOn javadoc
}

// Verify that source and javadoc jars are created
tasks.named('build') {
...
dependencies {
    ...
    // Additional dependencies specific to this app
    // (none needed for basic random data generation)
    
    // Optional: Add metrics or monitoring
    // implementation 'io.micrometer:micrometer-core:1.12.0'
}

application {
    mainClass = 'com.trading.app.random.RandomDataApplication'
    applicationName = 'random-data-app'
    ...
    main {
        contents {
            ...
            from('src/main/resources') {
                into 'config'
                include 'topology.yml', 'logback.xml'
            }
            from('README.md') {
                into 'docs'
            }
...
// Custom run task with arguments
tasks.register('runWithConfig', JavaExec) {
    group = 'application'
    description = 'Run with custom configuration file'
    mainClass = application.mainClass
    classpath = sourceSets.main.runtimeClasspath
    
    // Allow passing config file: ./gradlew runWithConfig -Pconfig=custom-topology.yml
    def configFile = project.findProperty('config') ?: 'topology.yml'
    args = [configFile]
...
tasks.register('createDockerfile') {
    group = 'docker'
    description = 'Create Dockerfile for this application'
    
    doLast {
        def dockerfileContent = """
FROM openjdk:17-slim

LABEL maintainer="dev@trading.com"
...
# Copy the fat JAR
COPY build/libs/${project.name}-all-${project.version}.jar app.jar

# Copy configuration
COPY src/main/resources/topology.yml /app/config/topology.yml
...
""".trim()
        ...
        file("${projectDir}/Dockerfile").text = dockerfileContent
        println "Dockerfile created at ${projectDir}/Dockerfile"
    }
}
...
tasks.register('buildInfo') {
    ...
    doLast {
        println ""
        println "=" * 80
        println "Build Information: ${project.name}"
        println "=" * 80
        println ""
        println "Version:     ${project.version}"
        ...
        println "  ./gradlew :${project.name}:run"
        println "  ./gradlew :${project.name}:runTopology"
        println "  ./gradlew :${project.name}:fatJar"
        println ""
        println "JAR Location:"
        println "  build/libs/${project.name}-${project.version}.jar"
        println "  build/libs/${project.name}-all-${project.version}.jar (fat jar)"
        println ""
        println "=" * 80
    }
}
...
dependencies {
    ...
    // JSON processing (already provided by core, but explicit for clarity)
    implementation 'com.fasterxml.jackson.core:jackson-databind:2.16.0'
...
application {
    mainClass = 'com.trading.app.jms.JmsStreamingApplication'
    applicationName = 'jms-streaming-app'
...
tasks.register('runTopology', JavaExec) {
    ...
    // System properties for JMS configuration
    // Can be overridden via -Djms.broker.url=... on command line
    systemProperties = [
        'jms.broker.url': System.getProperty('jms.broker.url', 'tcp://localhost:61616'),
        'jms.queue.name': System.getProperty('jms.queue.name', 'events.queue'),
        'jms.username': System.getProperty('jms.username', ''),
        'jms.password': System.getProperty('jms.password', '')
    ]
    
    // Add any additional system properties from command line
    System.properties.findAll { it.key.startsWith('topology.') }.each {
...
tasks.register('fatJar', Jar) {
    ...
    // Exclude signature files and duplicate files
    exclude 'META-INF/*.RSA', 'META-INF/*.SF', 'META-INF/*.DSA'
...
// Task to run with embedded Artemis (for testing)
tasks.register('runWithEmbeddedBroker', JavaExec) {
    group = 'application'
    description = 'Run with embedded Artemis broker for testing'
    mainClass = application.mainClass
    classpath = sourceSets.main.runtimeClasspath + sourceSets.test.runtimeClasspath
    
    jvmArgs = application.applicationDefaultJvmArgs + [
        '-Dembedded.broker=true',
        '-Dembedded.broker.port=61616'
    ]
...
# Environment variables for JMS configuration
ENV JMS_BROKER_URL="tcp://artemis:61616"
ENV JMS_QUEUE_NAME="events.queue"
ENV JMS_USERNAME=""
ENV JMS_PASSWORD=""
...
# Run the application
ENTRYPOINT ["sh", "-c", "java \$JAVA_OPTS -Djms.broker.url=\$JMS_BROKER_URL -Djms.queue.name=\$JMS_QUEUE_NAME -jar app.jar"]

HEALTHCHECK --interval=30s --timeout=3s --start-period=40s --retries=3 \\
  CMD echo "Health check placeholder - implement actual check"
""".trim()
...
tasks.register('createDockerCompose') {
    group = 'docker'
    description = 'Create docker-compose.yml for this application with Artemis'
...
services:
  artemis:
    ...
    environment:
      - AMQ_USER=admin
      - AMQ_PASSWORD=admin
      - AMQ_QUEUES=events.queue,test.queue
    ports:
      - "61616:61616"
      - "8161:8161"
    networks:
  ...
  jms-streaming-app:
    ...
    environment:
      - JMS_BROKER_URL=tcp://artemis:61616
      - JMS_QUEUE_NAME=events.queue
      - JMS_USERNAME=admin
      - JMS_PASSWORD=admin
...
""".trim()
        ...
        file("${projectDir}/docker-compose.yml").text = composeContent
        println "docker-compose.yml created at ${projectDir}/docker-compose.yml"
    }
}
...
tasks.register('buildInfo') {
    ...
    doLast {
        ...
        println "  - Core Framework: ${project(':storm-framework-core').version}"
        println "  - JMS API: jakarta.jms:jakarta.jms-api:3.1.0"
        println "  - Artemis Client: 2.31.2"
        println ""
        println "Run Commands:"
        println "  ./gradlew :${project.name}:run"
        println "  ./gradlew :${project.name}:runTopology"
        println "  ./gradlew :${project.name}:runWithEmbeddedBroker"
        println "  ./gradlew :${project.name}:fatJar"
        println "  ./gradlew :${project.name}:integrationTest"
        println ""
        println "Docker Commands:"
        println "  ./gradlew :${project.name}:createDockerfile"
        println "  ./gradlew :${project.name}:createDockerCompose"
...
## Summary of All Build Files

**Root Level:**

1. `build.gradle` - Root project configuration with common dependencies
2. `settings.gradle` - Module definitions and build cache settings

**storm-framework-core/** (Library Module)
3. `build.gradle` - Core framework with Maven publishing to Nexus, includes javadoc, sources, and POM configuration

**storm-app-random-data/** (Application Module)
4. `build.gradle` - Random data application with fat JAR creation, Docker support, and custom run tasks

**storm-app-jms-streaming/** (Application Module)
5. `build.gradle` - JMS streaming application with embedded broker support, Docker Compose, and integration tests

All build files include:

- ✅ Proper dependency management
- ✅ Maven publishing (core only)
- ✅ Fat JAR creation (apps)
- ✅ Docker support (apps)
- ✅ Custom run tasks
...
[^6_1]: Master-Implementation-Prompt_-Apache-Storm-Compati.md
